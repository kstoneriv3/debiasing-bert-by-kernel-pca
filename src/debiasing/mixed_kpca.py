...
from src.debiasing.torch_kpca import _to_torch_sparse, TorchRBF
import math

DTYPE = torch.float32
DEVICE = torch.device('cuda')  # 6.7x speed up by gpu

class MixedDebiasingKernelPCA:
    ...
    def fit(self, X, X_index=None):
        ...
        self.DX_fit_ = _to_torch_sparse(get_design_matrix(X_index))
    ...
    def orth_losses(self, X, X_orth, alpha):
        ...
        A = KoP.mm(self.alphas_)  # ; del KoP
        KPx = torch.sparse.mm(self.DX_fit_, self.kernel(self.X_fit_, X))
        B = self.alphas_.transpose(0, 1).mm(KPx)  # ; del KPx

        losses = (1. + alpha) * (Koo - 2. * Kox) + 2. * torch.einsum('ij, j, ji->i', A, self.lambdas_inv, B)
        losses /= (1. + alpha)
        return losses
    ...
    def debias(self, X, n_iter=30, lr=0.4, alpha=0.1, batch_size=16):
        ...
        X_orth = X.clone().detach().requires_grad_(True)

        self.X_fit_ = torch.from_numpy(self.X_fit_).float().to(DEVICE)
        self.kernel = self.add_autoreshape(TorchRBF)
